...
- **Recursive nz-submenu (Top Menu):** Nested menu trees must recurse through self-referencing **components** (`top-menu.component.ts`), never a single shared `ng-template` re-entered via `ngTemplateOutlet`. Angular resolves element injectors at the template's *declaration* site, so a shared template severs ng-zorro's parent `NzSubmenuService` discovery (fails NG0201; hover chains and vertical fly-outs break below level 1). `ngTemplateOutletInjector` does not fix this — verified empirically. The bar's root `<ul nz-menu nzMode="horizontal">` renders its level inline; deeper lists are recursive `<ul headerNav nz-menu nzMode="vertical">` instances whose host IS the list (strict `ul > li` DOM — Ant's direct-child CSS like `.ant-menu-horizontal > .ant-menu-submenu` dies otherwise) and whose own `nz-menu` provides a local menu context. Each submenu must also wrap its children in a real inner `<ul>` (content projects into a plain `div`; bare `<li>`s collapse Ant's submenu layout).
